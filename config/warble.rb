...
Warbler::Config.new do |config|
  ...
  #config.dirs = %w(app config lib log gems)
  ...
  config.excludes = FileList[*(
    %w(gems/cache gems/bin gems/doc) + %w(
      rspec
    ).map{|i| ["gems/gems/#{i}-*", "gems/specifications/#{i}-*"]}.flatten
  )] 
  ...
  if ENV['DEBUG_CONSOLE']
    ...
    config.webxml.rackup = <<-"RUBY"
      ENV['MERB_ENV'] = 'production'
      require 'rubygems'
      gem 'merb-core'
      require 'merb-core'
      $:.push File.join(ENV['JRUBY_HOME'],
        %w(lib ruby gems 1.8 gems rspec-1.2.2 lib))
      Merb.start
      path = "/tmp/t1"
      puts "[CONSOLE] echo 'ruby code' > \#{path}"
      loop do
        if File.exist?(path)
          begin
            puts "#=> " + eval(open(path).read).inspect
          rescue Exception => e
            puts "[E] " + ([e.message] + e.backtrace).join("\\n")
          ensure
            File.unlink path
          end
        end
